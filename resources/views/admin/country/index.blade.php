...
<div class="col-xs-12 col-sm-6 col-md-9" style="margin-top:20px">
		...
		<table class="table table-bordered">
			...
			<thead>
				...
				<th>Title <i class="fa fa-caret-up sort" id="country_name"></i></th>
                    ...
                    <div id="error-msg1"></div>
		      		<form data-toggle="validator" action="{{url('/admin/country/create')}}" method="POST">
		      			...
		      			<div class="form-group">
							...
							<input type="text" name="country_name" class="form-control" data-error="Please enter title." required />
...
$(document).on("click",".submit-create",function(e){
   loader();
    ...
    var country_name = $("#create-country").find("input[name='country_name']").val();
    ...
    if(country_name != '' && code != ''){
        ...
        $.ajax({
            ...
            type:'POST',
            url:  form_action,
            data:{ country_name:country_name, code:code, _token:CSRF_TOKEN }

        }).done(function(res){
			loader();
			if(res.status == 'true' || res.status == true)
			{				
				$("#create-country").find("input[name='country_name']").val('');
        ...
        loader();
		loadError('You are missing country or code.','error-msg1');
...
$("body").on("click",".edit-country",function(){
    ...
    var country_name = $(this).parent("td").prev("td").prev("td").text();

    var code = $(this).parent("td").prev("td").text();


    $("#edit-country").find("input[name='country_name']").val(country_name);
...
$(".submit-edit").click(function(e){
    ...
    var country_name = $("#edit-country").find("input[name='country_name']").val();
    ...
    if(country_name != '' && code != ''){
        ...
        $.ajax({
            ...
            data:{country_name:country_name, code:code, id:id, _token :CSRF_TOKEN }

        }).done(function(res){
			loader();
			if(res.status == 'true' || res.status == true)
			{				
				$("#edit-country").find("input[name='country_name']").val('');

				$("#edit-country").find("input[name='code']").val('');

				$("#edit-country").find("input[select='status']").val('');
        ...
        loader();
		loadError('You are missing country or code.','error-msg2');
